...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(trim($basePrice) == "" || empty($basePrice)){
        $_SESSION["error"] = "Product Base Price field is empty";
    ...
    if(trim($numberOfStocks) == "" || empty($numberOfStocks)){
        $_SESSION["error"] = "Product Number of Stocks field is empty";
    ...
    if(trim($unitPrice) == "" || empty($unitPrice)){
        $_SESSION["error"] = "Product Unit Price field is empty";
    ...
    if(trim($totalPrice) == "" || empty($totalPrice)){
        $_SESSION["error"] = "Product Total Price field is empty";
...
function processImage($id){
    ...
    $conn = $db->connectDB();
    $sql  = $sql = "UPDATE products SET image_url = :p_image_url WHERE id = :p_id";
    $stmt = $conn->prepare($sql);
